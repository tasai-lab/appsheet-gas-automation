...
/**
 * 🚀 クイックテスト: promptType='通常' 
 * 引数なしで直接実行可能
 * GASエディタから実行ボタンをクリックするだけでテストできます
 *
 * @return {Object} 処理結果
 */
function quickTestNormal() {
  Logger.log('='.repeat(60));
  Logger.log('🚀 クイックテスト: promptType="通常"');
  Logger.log('='.repeat(60));
  ...
  const result = processClientQA(
    '今後必要な支援内容を具体的に提案してください。',
    {
      promptType: '通常',
      userId: 'USER001',
      userBasicInfo: '氏名: 山田花子\n年齢: 82歳\n要介護度: 要介護3',
      referenceData: '訪問記録: 歩行が不安定、血圧150/90'
    }
  );

  Logger.log('✅ 成功');
  Logger.log('📝 回答: ' + result.answer.substring(0, 200) + '...');
...
function quickTestDocument() {
  Logger.log('='.repeat(60));
  Logger.log('🚀 クイックテスト: promptType="外部文章"');
  Logger.log('='.repeat(60));
...
/**
 * テスト用関数: promptType='通常' での質疑応答
 * promptType引数を使った新しい推奨形式のテスト
 *
 * @return {Object} 処理結果
 */
function testPromptTypeNormal() {
  Logger.log('='.repeat(60));
  Logger.log('🧪 promptType="通常" での質疑応答テスト');
  Logger.log('='.repeat(60));
...
function testPromptTypeDocument() {
  Logger.log('='.repeat(60));
  Logger.log('🧪 promptType="外部文章" での質疑応答テスト');
  Logger.log('='.repeat(60));
