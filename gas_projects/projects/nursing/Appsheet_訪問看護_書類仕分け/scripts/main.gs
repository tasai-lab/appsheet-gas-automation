...
function callGeminiApi(prompt, log, model = 'gemini-2.5-pro') {
  ...
  log(`Vertex AI API呼び出し開始: モデル=${model}`);
  ...
  log(`Vertex AI API 応答: ${responseCode}`);

  if (responseCode >= 400) throw new Error(`Vertex AI API Error (${responseCode}): ${responseText}`);
