...
function resetApiCallCounter() {
  SYSTEM_CONFIG._apiCallCounter = 0;
  logStructured(LOG_LEVEL.INFO, 'API呼び出しカウンターをリセット');
}

/**
 * API呼び出しカウンターを増加し、制限をチェック
 * @param {string} apiType - APIの種類 ('Vertex_AI', 'Google_AI', 'AppSheet')
 * @throws {Error} - 制限を超えた場合
 */
function incrementApiCallCounter(apiType) {
  SYSTEM_CONFIG._apiCallCounter++;

  logStructured(LOG_LEVEL.INFO, `API呼び出し: ${apiType} (${SYSTEM_CONFIG._apiCallCounter}/${SYSTEM_CONFIG.maxApiCallsPerExecution})`, {
    apiType: apiType,
    currentCount: SYSTEM_CONFIG._apiCallCounter,
    maxCount: SYSTEM_CONFIG.maxApiCallsPerExecution
  });

  if (SYSTEM_CONFIG._apiCallCounter > SYSTEM_CONFIG.maxApiCallsPerExecution) {
    const errorMsg = `API呼び出し制限超過: ${SYSTEM_CONFIG._apiCallCounter}回 (上限: ${SYSTEM_CONFIG.maxApiCallsPerExecution}回)`;
    logStructured(LOG_LEVEL.ERROR, errorMsg, {
      apiType: apiType,
      currentCount: SYSTEM_CONFIG._apiCallCounter,
      maxCount: SYSTEM_CONFIG.maxApiCallsPerExecution
    });
    throw new Error(errorMsg + ' - 1回の処理で過度なAPI呼び出しが発生しています。処理を中止します。');
  }
}

/**
 * 現在のAPI呼び出し回数を取得
 * @returns {number} - 現在の呼び出し回数
 */
function getApiCallCount() {
  return SYSTEM_CONFIG._apiCallCounter;
}
...
/**
 * ★★★ Google AI Studio APIは完全廃止しました ★★★
 *
 * 理由: 無料枠超過により90%エラー + 200,000リクエスト/日の問題発生
 * 修正日: 2025-10-18
 * 今後: すべてVertex AI APIのみ使用
 *
 * APIキーは削除済み。GEMINI_CONFIGは互換性のため残していますが、
 * 実際のGoogle AI Studio API呼び出し関数は削除済みです。
 */
const GEMINI_CONFIG = {
  ...
  // Vertex AI使用フラグ - 常にtrueを返す
  useVertexAI: function() {
    return true;  // ★強制的にVertex AIのみ使用
  }
};
