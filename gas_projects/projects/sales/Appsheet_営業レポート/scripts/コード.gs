...
class Config {
  ...
  static get AI_OPTIONS() {
    ...
    return {
      ...
      // GEMINI_API_KEY: '',  // ★削除済み - Vertex AIはOAuth2認証を使用
...
class AiAnalysisService {
  ...
  static generateExecutiveSummary(reportData) {
    ...
    // ★★★ Vertex AI使用のため、APIキーチェックは不要（OAuth2認証） ★★★
    // 修正日: 2025-10-18
    if (!Config.AI_OPTIONS.GCP_PROJECT_ID) {

      LoggerUtil.warn("GCP Project ID is not configured.");
...
`;
  ...
  static callGeminiApi(prompt) {
    ...
    const apiUrl = `https://${Config.AI_OPTIONS.GCP_LOCATION}-aiplatform.googleapis.com/v1/projects/${Config.AI_OPTIONS.GCP_PROJECT_ID}/locations/${Config.AI_OPTIONS.GCP_LOCATION}/publishers/google/models/${Config.AI_OPTIONS.MODEL_NAME}:generateContent`;
