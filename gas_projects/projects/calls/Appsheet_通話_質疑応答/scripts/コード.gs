...
const SETTINGS = {
  ...
  // Webhookで指定するキーワードと、実際に使用するVertex AI Geminiモデル名を定義します。

  MODEL_MAPPING: {

    "しっかり": "gemini-2.5-pro", // Vertex AI: 高度な推論 + 思考モード

    "はやい": "gemini-2.5-flash" // Vertex AI: 高速・コスト効率 + 思考モード
...
/**
 * Vertex AI（Gemini）を呼び出し、回答を生成する
 *
...
function generateAnswerWithGemini(logCollector, promptText, callSummary, callTranscript, call_info, model) {
  ...
  log(logCollector, 'Vertex AI (Gemini) 処理開始...');
  log(logCollector, `使用モデル: ${model}`);
...
`;
  ...
  try {
    // Vertex AI クライアントを作成（gemini_client.gs の GeminiClient クラスを使用）
    const client = createGeminiClient(model, {
      temperature: 0.3,
      maxOutputTokens: 20000,
      enableThinking: false  // 質疑応答では推論モード不要
    });

    log(logCollector, `プロンプトの長さ: ${prompt.length}文字`);

    // Vertex AI API呼び出し（OAuth2認証）
    const answerText = client.generateText(prompt, logCollector);

    log(logCollector, `AIからの生成回答（長さ: ${answerText.length}文字）`);
    log(logCollector, 'Vertex AI 処理正常終了。');

    return answerText;

  } catch (error) {
    log(logCollector, `[エラー] Vertex AI処理に失敗しました: ${error.toString()}`);
