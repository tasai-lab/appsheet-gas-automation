...
function getConfig() {
  ...
  return {
    ...
    // このプロジェクトは既にVertex AI専用ですが、設定に痕跡が残っていたため削除
    // geminiApiKey: props.getProperty('GEMINI_API_KEY') || '',  // ★削除済み - 未使用
...
function setupScriptProperties() {
  ...
  const settings = {
    ...
    // 修正日: 2025-10-18
    // このプロジェクトは既にVertex AI専用（OAuth2認証使用）
    // 'GEMINI_API_KEY': '',  // ★削除済み - 未使用
